...
<body id="page-top">
    <?php
        ...
        // create instance of card in the database
		if (isset($_POST['enter_card'])) {
			$name = $_POST['name'];
            if (isset($_POST['color'])) {
                $color = $_POST['color'];
			}
            $stackID = $_SESSION['currentStackID'];
			$startDate = "".date('Y-m-d');

            // insert card instance into database
            $stackStmt = $con->prepare("INSERT into CARD values (null, ?, ?, ?, ?, 1);");
            $stackStmt->execute(array($name, $color, $stackID, $startDate));
            $stackStmt->closeCursor();

            // get project id of project we just created
			$lastStmt = $con->prepare("SELECT @@IDENTITY AS card_id FROM CARD;");
			$lastStmt->execute();
			$lastRow = $lastStmt->fetch(PDO::FETCH_OBJ);
            $lastStmt->closeCursor();
			$cardID = $lastRow->card_id;
			...
			foreach($_SESSION['memberArr'] as $currentMember) {
				...
				$accessStmt = $con->prepare("insert into ASSIGNMENT values (null, ?, ?, ?, ?);");
				$accessStmt->execute(array($name, $memberID, $cardID, $startDate));
			}
            $arrStmt->closeCursor();
            ...
            <div id="content">
                <nav class="navbar navbar-light navbar-expand bg-white shadow mb-4 topbar static-top">
                    <div class="container-fluid"><button class="btn btn-link d-md-none rounded-circle me-3"
                        ...
                        <h3 class="text-dark mb-1">Create Card</h3>
                            ...
                            <div class="col">
                                <div class="card shadow mb-4">
                                    <div class="card-header py-3" style="background: var(--bs-gray-200);">
                                        <h6 class="text-primary m-0 fw-bold">Create a Card</h6>
                                    </div>
                                    <div class="card-body" style="background: var(--bs-gray-100);">
                                        <form class="d-xxl-flex justify-content-xxl-center" action="createCard.php"
                                            method="post">
                                            <div class="col">
                                                <div class="row">
                                                    <div class="col"><label class="col-form-label fs-6">Card Name<input class="form-control" type="text" name="name" value="New Card"></label></div>
                                                </div>
                                                ...
                                                <div class="row">
                                                    <div class="col d-xxl-flex justify-content-xxl-end">
                                                        <a class="btn btn-primary"
                                                            ...
                                                            <input
                                                            class="btn btn-primary" type="submit" name="enter_card"
                            ...
                            <div class="col">
                                <div class="card shadow mb-4">
                                    ...
                                    <div class="card-body" style="background: var(--bs-gray-100); height: 305px;">
                                        ...
                                        <form class="d-xxl-flex justify-content-xxl-center" action="createCard.php" method="post">
                                            <div class="col">
                                                <div class="row">
